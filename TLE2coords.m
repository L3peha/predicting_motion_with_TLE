function [rVec, vVec] = TLE2coords(TLE)
...
if n == 2
    ...
    for i = 1:1:n-1
        t = (sgp(i+1,9)-sgp(i,9))*24*60*accur;
        if t==0
            continue;
        end
        for j = 1:1:t+1
...
%            [rast(j,:), velos(j,:)] = SGP4(t0, sgp(1,1), sgp(1,2), sgp(1,3), sgp(1,4), sgp(1,5), sgp(1,6), sgp(1,7), accur+time, sgp(1,8));
 %           if j==floor(t+1)
  %              dobav = (t+1-j)*60;
   %             for k =1:1:dobav
    %                [r(j+k,:), v(j+k,:)] = SGP4(t0, sgp(i,1), sgp(i,2), sgp(i,3), sgp(i,4), sgp(i,5), sgp(i,6), sgp(i,7), time + k/60, sgp(i,8));
     %               [rast(j+k,:), velos(j+k,:)] = SGP4(t0, sgp(1,1), sgp(1,2), sgp(1,3), sgp(1,4), sgp(1,5), sgp(1,6), sgp(1,7), accur+time + k/60, sgp(1,8));
      %          end
       %     end
        end
        %accur = accur + t;
